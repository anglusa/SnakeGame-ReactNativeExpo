...
const Game = () => {
    ...
    function moveSnake() {
        ...
        if (testGameOver(head)) {
            setIsGameOver(true);
            Haptics.notificationsAsync(Haptics.NotificationsFeedackType.Error);
            return;
        }
        if (testEatsFood(head, food)) {
            Haptics.notificationsAsync(Haptics.NotificationsFeedackType.Success);
...
function testGameOver(snakeHead) {
    return (
        snakeHead.x < limits.mixX ||
        snakeHead.x > limits.maxX ||
        snakeHead.x < limits.minY ||
        snakeHead.x > limits.maxY
    );
}
